...
def api_quiz():
    ...
    return jsonify({
        ...
        "personality_meanings": personality_meanings
    })
...
def api_predict():
    ...
    # Need at least one feature to predict, but we'll try transforming
    x_skills = mlb_skills.transform([skills]) if skills else np.zeros((1, len(mlb_skills.classes_)))
    x_personality = mlb_personality.transform([personalities]) if personalities else np.zeros((1, len(mlb_personality.classes_)))
    
    # If the user sends empty lists but the model expects the exact number of features, 
    # transform takes care of it by matching classes. 
    # It's safer to ensure they match model's expected input shape.
    x = np.hstack((x_skills, x_personality))
